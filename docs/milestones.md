...
| 2026-08-13 | `3154853` | **Aufgaben lassen sich in andere Aufgaben ziehen** (`spec.md` §2.4): Mitte der Zeile = hinein, Drittel = daneben, keine Tiefengrenze, Kreisfall wird gar nicht erst angeboten. `test_unteraufgabe.js` mit 21 Zusicherungen, per `git stash` gegengeprueft
| 2026-08-13 | `770f999` | **Palette "Koernig" uebernommen**: fuenf Farben vom Nutzer, abgeleitete Toene als solche gekennzeichnet, alle drei Theme-Bloecke umgestellt, `spec.md` §3 nachgezogen. Voller Lauf 55 gruen / 0 rot, `test_contrast` 0 Durchgefallene in beiden Modi
| 2026-08-14 | `96d4393` | **Der Prototyp ist alltagstauglich - und die Nutzung findet Fehler.** Datenschicht mit localStorage, Export/Import als JSON mit Schema-Version 1 und Sicherung des letzten guten Standes (bb2b9f8); dazu vier Fehler, drei davon vom Nutzer beim Benutzen gefunden: ein unlesbarer Stand wurde still ueberschrieben, die Kopfzeile sprang beim Abhaken der letzten Aufgabe (schriftabhaengig, im Container nicht reproduzierbar), und die Gegenstauchung der Spalten-Bewegung hob die Skalierung nur an den Enden auf - 12,5% Verzerrung mitten in der Bewegung, beim Oeffnen UND beim Schliessen. Vier neue Pruefungen (test_speichern, test_kopf_stabil, test_flip_sauber, test_kaestchen), alle per git stash gegengeprueft |
| 2026-08-14 | `00b9e71` | **Workflow beschleunigt, ohne eine Pruefung abzubauen:** Der Laeufer merkt sich den gemessenen Stand ueber eine Signatur (8 min -> 0 s bei unveraendertem Code), decisions.md bekommt ein erzeugtes Verzeichnis ueber 85 Eintraege, der Lernkurven-Abgleich laeuft in session-check.sh mit. Leitgedanke: Struktur statt Streichen - keine Pruefung entfernt, keine Zusicherung entschaerft |
| 2026-08-13 | `3cb5703` | **Die Stimmigkeit wird jetzt gemessen statt behauptet:** `test_stimmigkeit.js` prueft Schrift-, Dauer-, Kurven- und Radienskala sowie das Verhaeltnis von Bild- zu Wortmarke an den **gerenderten** Elementen. Erster Lauf fand drei Brueche (eine dreizehnte Uebergangsdauer, eine undokumentierte vierte Bewegungskurve, neun Bewegungen auf der Browser-Vorgabe) - darunter dieselbe Animation auf zwei verschiedenen Kurven. Dazu die vier Wuensche des Nutzers: neue Liste blendet mit `block-in` ein, Schalter auf `ease-lauf`, Wortmarke nachgemessen (Verhaeltnis 1,88, kein Handlungsbedarf). Voller Lauf 56 gruen / 0 rot |
